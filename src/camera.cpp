    ...
    : m_time_window(time_window)
{
    ...
    m_origin = eye.origin();
    m_w = unit_vector(origin - eye.direction());
    m_u = unit_vector(cross(up, w));
    m_v = cross(m_w, m_u);

    // fix viewport 1 unit from camera
    m_lower_left_corner = origin - half_width * focus_dist * m_u - half_height * focus_dist * m_v - focus_dist * m_w;
...
Ray Camera::to_viewport(float s, float t) const {
    ...
    Time const time = m_shutter_window.start + rand() * m_shutter_window.delta();

    return Ray(m_origin + offset,
               m_lower_left_corner + s * m_horizontal + t * m_vertical - m_origin - m_offset,
               time);
}
